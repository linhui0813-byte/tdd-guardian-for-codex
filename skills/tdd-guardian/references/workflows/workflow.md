...
description: |
  ...
  assistant: |
    $ARGUMENTS is empty. I use ask the user to elicit a plain-language task description, then apply the chain starting at $tdd-guardian plan.
  </example>
...
1. Follow `references/policies/policy-core/policy.md` throughout, and `references/policies/workflow/policy.md` for stage ordering and stop conditions.
...
4. Every stage persists its artifact under `.codex/tdd-guardian/` so the next stage can resume without re-prompting.

## Steps

### Step 1 — Config + input

1. Follow `workflows/shared/load-config.md` to load and validate config.
...
3. If `$ARGUMENTS` is empty, use `ask the user`:
   ```
...
Invoke `$tdd-guardian plan <validated description>`. This applies `tdd-planner` and writes `.codex/tdd-guardian/plan-{ts}.md`.
...
Invoke `$tdd-guardian design-tests <plan path>`. This applies `tdd-test-designer` and writes `.codex/tdd-guardian/tests-{ts}.md`.
...
- Plan: `.codex/tdd-guardian/plan-{ts}.md`
- Tests: `.codex/tdd-guardian/tests-{ts}.md`
- Coverage: `.codex/tdd-guardian/coverage-{ts}.md`
- Mutation: `.codex/tdd-guardian/mutation-{ts}.md` (if run)
- Review: `.codex/tdd-guardian/review-{ts}.md`

## Next step

Gates are green. Review the implementation and commit when ready.
The Stop hook may still run the gate runner on commit; use `TDD_GUARD_BYPASS=1` only with explicit user consent.
```
