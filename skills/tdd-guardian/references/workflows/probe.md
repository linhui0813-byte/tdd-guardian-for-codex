...
Follow `workflows/shared/load-config.md`. Stop on missing or disabled config. Surface every validation warning — they describe ways the gate can silently do nothing.
...
Use the lane's `probeCommand` if set. Otherwise infer one from the lane's `command` using the probe table in `workflows/shared/detect-tooling.md` and the per-ecosystem files in the `tooling-catalog` policy.
...
**Read `ever_had_tests` from `.codex/tdd-guardian/state.json` to tell `bootstrap` from `empty`.** They look identical at the probe and mean opposite things: one is a brand-new project behaving as designed, the other is a regression. Diagnosing a greenfield repo as "your glob is wrong" sends the user hunting for a bug that does not exist.
...
- `timeoutMs` is consistent with the trigger — over 15 minutes on `stop` is a misconfiguration.
...
**Workspace**: {pwd}
**Config**: `.codex/tdd-guardian/config.json` (schema v{n})

| Lane | Verdict | Tests found | Trigger | Coverage | Probe |
|------|---------|-------------|---------|----------|-------|
| unit | verified | 148 | stop, commit | include | `pnpm exec vitest list` |
