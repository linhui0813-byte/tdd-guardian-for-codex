---
description: "Shared: load .codex/tdd-guardian/config.json, migrate v1 to v2, validate lanes, fail loudly if missing"
...
Load, migrate, and validate the per-project configuration at `.codex/tdd-guardian/config.json`. All TDD Guardian commands depend on this file — a missing or malformed config is a hard stop.
...
Resolve `${WORKSPACE}/.codex/tdd-guardian/config.json`, where `${WORKSPACE}` is the current working directory of the invoking command. Read that absolute path.
...
```
TDD Guardian config not found at .codex/tdd-guardian/config.json.
...
```
TDD Guardian config at .codex/tdd-guardian/config.json is not valid JSON.
...
| `mutationGateOn` | array | no | Defaults to `["stop"]` |
...
| `enforceOnStop` | boolean | no | Whether the Stop hook runs lanes |
...
| `gateOn` | array | no | Subset of `stop`, `commit`, `push`, `manual`. Omitted defaults to `["stop","commit"]`; explicit `[]` means manual-only |
...
- A lane on `stop` with a timeout over 15 minutes — long suites belong on `push`.
