...
3. **Blocking is opt-in.** `enforceOnStop` and `blockCommitWithoutFreshGate` default to `false`.
...
This is the best possible TDD starting point, not a failure. Offer to scaffold, using `ask the user`:
...
Follow `workflows/shared/detect-tooling.md` in full. Its evidence order is CI config → manifests → test topology → dry-run probe → ask. Use the `tooling-catalog` policy for per-ecosystem facts, reading only the reference file for the ecosystems you find.
...
Apply `references/policies/lane-policy/policy.md`:

| Lane characteristics | `gateOn` |
|----------------------|----------|
| Under ~60s, no external services | `["stop", "commit"]` |
...
Ask two separate questions for the blocking switches, defaulting both to off:

```
question: "Run fast test lanes automatically at the end of each Codex turn?"
header: "Turn gate"
options:
  - "Off (recommended)" — keep enforceOnStop false
  - "On" — set enforceOnStop true

question: "Block commit, push, pull-request, and publish commands when their required gates are stale?"
header: "Command gate"
options:
  - "Off (recommended)" — keep blockCommitWithoutFreshGate false
  - "On" — set blockCommitWithoutFreshGate true
```
...
Write `.codex/tdd-guardian/config.json`:
...
**Workspace**: {pwd}
**Config**: `.codex/tdd-guardian/config.json` (schema v2)
**Enforcement**: stop {on|off} / commit-push blocking {on|off}

## Lanes

| Lane | Command | Trigger | Coverage | Probe | Tests found |
|------|---------|---------|----------|-------|-------------|
| unit | `pnpm exec vitest run --coverage` | stop, commit | `coverage/coverage-summary.json` | verified | 148 |
...
- Output: one markdown report, plus `.codex/tdd-guardian/config.json` and a `.gitignore` entry.
...
Read references:
- `references/policies/tooling-catalog/policy.md`
- `references/policies/lane-policy/policy.md`
- `references/policies/init/policy.md`
- `references/policies/policy-core/policy.md`
