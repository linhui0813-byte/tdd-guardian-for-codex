...
Create `.codex/tdd-guardian/config.json` describing every test lane the repository actually has.

## Steps

1. **Detect** — follow `workflows/shared/detect-tooling.md`. Evidence order is CI config → manifests → test topology → dry-run probe → ask. Use the `tooling-catalog` skill for per-ecosystem facts.
...
5. **Write** the config, then append `.codex/tdd-guardian/state.json` to `.gitignore`.
...
| `gateOn` | `stop`, `commit`, `push`, `manual` |
...
| Question | Where |
|----------|-------|
| How is a repo's tooling detected and verified? | `workflows/shared/detect-tooling.md` |
| What runner and coverage tool does language X use? | `references/policies/tooling-catalog/policy.md` |
| Why does this lane get this trigger? | `references/policies/lane-policy/policy.md` |
