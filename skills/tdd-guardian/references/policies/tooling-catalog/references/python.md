...
|------|---------|---------|
| unit | `<run> pytest -m "not integration and not e2e"` | `["stop", "commit"]` |
...
These orchestrate environments; they are not runners. Read `tox.ini` / `noxfile.py` to find the underlying command and configure **that** as the lane. A `tox` lane re-creates virtualenvs on every gate run, which is far too slow for `stop`.
