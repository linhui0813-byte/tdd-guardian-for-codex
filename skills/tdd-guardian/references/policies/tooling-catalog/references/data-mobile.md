...
|------|----------|---------|---------|
| unit | `test/` | `flutter test` | `["stop", "commit"]` |
...
`package_coverage()` re-installs the package into a temporary library on every call, so it is slow. Keep it off `stop`.
...
- **covr re-installs the package on every run.** It is correct but slow; do not put it on `stop`.
