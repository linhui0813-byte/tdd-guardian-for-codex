...
`sbt` boots a JVM and resolves dependencies on first run — often 30s+ before a single test runs. Prefer a longer `timeoutMs` and consider `["commit"]` over `["stop"]`.
