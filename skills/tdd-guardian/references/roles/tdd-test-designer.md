...
description: |
  ...
  assistant: "I'll apply the tdd-test-designer to design behavior-driven tests with real file fixtures — no mocking the fs module — covering all edge cases and specifying Level 1-5 assertions for each."
  </example>
model: inherit
references:
...
| `Write` | Writing the matrix to `.codex/tdd-guardian/tests-{timestamp}.md` — that file only |
...
Write the matrix to `.codex/tdd-guardian/tests-{YYYYMMDD-HHMMSS}.md` and return it. One section per unit, one block per case:
