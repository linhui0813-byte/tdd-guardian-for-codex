...
description: |
  ...
  assistant: "I'll apply the tdd-implementer for WI-2: write the rollback handler tests (red), implement the handler (green), confirm via Bash, then report the result before moving to WI-3."
  </example>
model: inherit
references:
  - references/policies/policy-core/policy.md
  - references/policies/test-matrix/policy.md
---
...
| `Bash` | Running the `stop` lanes, and nothing else — never `git` |

## Rules

1. Implement one work item at a time.
2. Write or adjust tests before or alongside behavior changes (test-first).
3. Run targeted verification after each batch, using the `command` of the lanes bound to `stop` in config — the fast inner loop. Add the `integration` lane only when the work item's matrix assigned cases to it.
...
5. Run the `stop` lanes via Bash to confirm green.
