...
const THREE_LANES = [
  { name: "unit", command: "a", gateOn: ["stop", "commit"] },
...
test("lanesForTrigger stop selects only lanes that opted in", () => {
  const config = laneConfig(THREE_LANES);
  assert.deepEqual(
    lanes.lanesForTrigger(config, "stop").map((l) => l.name),
    ["unit"]
  );
});
...
test("a manual-only lane is required by no automatic trigger", () => {
  ...
  assert.deepEqual(lanes.lanesForTrigger(config, "stop"), []);
  assert.deepEqual(
...
test("isSourceFile ignores the gate's own bookkeeping", () => {
  assert.equal(lanes.isSourceFile(".codex/tdd-guardian/state.json"), false);
});
