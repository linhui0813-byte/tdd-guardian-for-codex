...
//
// Hook output follows Codex's PreToolUse schema.
...
function buildReason(action, freshness, config) {
  ...
  lines.push(
    "",
    `Ask Codex to use $tdd-guardian to run the ${action} gate, then retry.`,
...
function main() {
  ...
  if (loaded.errors.length) {
    ...
    deny(
      [
        "TDD Guardian config is invalid, so gate freshness cannot be verified.",
        "",
        ...loaded.errors,
        "",
        "Fix the config or ask Codex to use $tdd-guardian to initialize this repository.",
      ].join("\n")
    );
    return;
  }
