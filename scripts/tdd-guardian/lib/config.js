...
// what lets a slow e2e suite gate `git push` without running at the end of every
// Codex turn, and lets an uninstrumented e2e lane stay out of the coverage maths.
//
...
//   stop          — the Codex Stop hook, at the end of a turn
//   commit        — `git commit`
//   push          — `git push`, `gh pr create|merge`, package publish
//   manual        — only when Codex runs the TDD Guardian skill on demand
const TRIGGERS = ["stop", "commit", "push", "manual"];
...
function normalizeLane(raw, index) {
  ...
  const gateOnRaw = Array.isArray(raw?.gateOn) ? (raw.gateOn.length ? raw.gateOn : ["manual"]) : ["stop", "commit"];
  const gateOn = [];
 ...
 * The Codex initialization workflow uses a single instrumented command instead.
 *
...
function migrate(raw) {
  ...
  notes.push(
    `Migrated schema v1 config to v2: created lane "unit" from testCommand${coverageCommand ? " + coverageCommand" : ""}. ` +
      `Ask Codex to use $tdd-guardian to initialize again if you want integration and e2e lanes.`
  );
...
function validate(rawInput) {
  ...
  const hasNativeStopFlag = Object.prototype.hasOwnProperty.call(migrated, "enforceOnStop");
  config.enabled = config.enabled !== false;
  config.enforceOnStop = hasNativeStopFlag
    ? migrated.enforceOnStop === true
    : migrated.enforceOnTaskCompleted === true;
  ...
  if (lanes.length === 0) {
    ...
    errors.push(
      `No lanes configured. TDD Guardian needs at least one lane.\n` +
        ...
        `     "lanes": [{ "name": "unit", "command": "<your test command>", "gateOn": ["stop", "commit"] }]\n` +
        `  2. Install a test runner, then ask Codex to use $tdd-guardian to initialize this repository.\n` +
        `  3. Delete .codex/tdd-guardian/config.json — with no config the plugin stays silent and nothing is blocked.`
    );
  }
  ...
  // A lane gating Stop with a long timeout will stall every Codex turn.
  for (const lane of lanes) {
    if (lane.gateOn.includes("stop") && lane.timeoutMs > 900000) {
      warnings.push(
        `Lane '${lane.name}' gates Stop with a ${Math.round(lane.timeoutMs / 60000)} min timeout. ` +
...
function configPath(cwd) {
  return path.join(cwd, ".codex", "tdd-guardian", "config.json");
}

function statePath(cwd) {
  return path.join(cwd, ".codex", "tdd-guardian", "state.json");
}
...
function load(cwd) {
  ...
  try {
    ...
    return {
      config: null,
      errors: [`TDD Guardian config at .codex/tdd-guardian/config.json is not valid JSON.\n\nParser error: ${err.message}`],
      warnings: [],
      notes: [],
      exists: true,
    };
